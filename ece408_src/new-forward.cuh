...
__global__ void first_kernel(float * __restrict__ y, const float * __restrict__ x){
...
__global__ void second_kernel(float * __restrict__ y, const float * __restrict__ x){
    int t = blockIdx.x * BLOCK_SIZE_2+threadIdx.x;
    int c = threadIdx.y;
    //int temp = c / 6; //which c to load
    //int temp2 = c % 6; //which out of 5 in c to load
    int b = blockIdx.y;
...
void forward<gpu, float>(mshadow::Tensor<gpu, 4, float> &y, const mshadow::Tensor<gpu, 4, float> &x, const mshadow::Tensor<gpu, 4, float> &w)
{
    ...
    int batch = 1000;
    const int streamNum = B/batch;
    cout <<"stream num = "<<streamNum<<endl;
    ...
    if(H == 64){
        ...
        int batch_size_y = 21600 * batch;
        int batch_size_x = 4096 * batch;
        for(int i = 0; i < streamNum; i++){
            //cout<<"stream "<<i<<endl;
            dim3 gridDim(grid_dim_x, batch,1);
            dim3 blockDim(BLOCK_SIZE,5,1);
            cudaStreamCreate(&streams[i]);
            first_kernel<<<gridDim, blockDim, 0, streams[i]>>>(&y.dptr_[i*batch_size_y], &x.dptr_[i*batch_size_x]);
        }
        ...
        int batch_size_y = 10816 * batch;
        int batch_size_x = 5400 * batch;
        
        for(int i = 0; i < streamNum; i++){
            //cout<<"stream "<<i<<endl;
            dim3 gridDim(grid_dim_x, batch,1);
            dim3 blockDim(BLOCK_SIZE_2, 6,1);
            cudaStreamCreate(&streams[i]);
            second_kernel<<<gridDim,blockDim, 0, streams[i]>>>(&y.dptr_[i*batch_size_y], &x.dptr_[i*batch_size_x]);
            MSHADOW_CUDA_CALL(cudaDeviceSynchronize());
        }
